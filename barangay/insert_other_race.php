...
die("Connection failed: " . mysqli_connect_error());
    ...
    //insert training
    $sql = 'INSERT INTO other_race (cid, type, race_point, date_start, time_release, date_expire, coord_long, coord_lat) VALUES ("'.$login_club.'","'.$encrypt_type.'","'.$encrypt_point.'","'.$datestart.'","'.$datetimerelease.'","'.$datetimeexpire.'","'.$encrypt_long.'","'.$encrypt_lat.'")';
     
    $result = mysqli_query($db, $sql) or die('Error querying database.');
    echo "<script type= 'text/javascript'>alert('New Race Schedule Added Successfully!');</script>";	
	  header("refresh:0; url=add-other-race");
    mysqli_close($db);
   }
?>