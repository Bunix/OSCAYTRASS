...
<?php 
$countquery = "select count(rid) as crid from race_entries where rid = '".$_GET['result_id']."'";
$trresult = mysqli_query($db,$countquery);
$trrow = mysqli_fetch_array($trresult);

$countquery2 = "select count(rid) as crid from race_entries where rid = '".$_GET['result_id']."' and clock = 1";
...
<div class="container-fluid">
  ...
  <tfoot>
    ...
    <?php                        
       ...
       $dsql = "SELECT *, (TIME_TO_SEC(time_arrived) - TIME_TO_SEC(time_release))/60 as minutes, distance/((TIME_TO_SEC(time_arrived) - TIME_TO_SEC(time_release))/60) as speed FROM race_result where rid = '".$_GET['result_id']."' and cid = '".$cid."' order by speed desc";
