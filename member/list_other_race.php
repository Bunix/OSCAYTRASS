...
$cid = mysqli_escape_string($db, $_GET["view_other_id"]); 

$query ="SELECT * FROM other_race where cid = '$cid' and date_expire > sysdate() order by id desc";  
              ...
              <div class="table-responsive">
                <table class="table table-striped table-bordered" id="dataTable" width="100%" cellspacing="0"s>
                  ...
                  <form>
                          ...
                          {  
                             $noquery ="SELECT count(rid) as entry FROM other_race_entries where cid = '".$cid."' and rid = '".$row["id"]."'";  
...
<br>
          ...
          <?php 
          $query ="SELECT * FROM other_race where cid = '".$cid."' and date_expire < sysdate() order by date_expire desc";  
          $result = mysqli_query($db, $query);   ?>
...
{
        window.location.href='list-other-race-entries?list_entries='+id+'<?php
...
function rfid_id(id)
{
        window.location.href='rfid-other-race-clock?rfid_id='+id+'<?php
        $codequery ="SELECT * FROM code order by rand() limit 1";  
        $coderesult = mysqli_query($db, $codequery);
        $coderow = mysqli_fetch_array($coderesult);
        $code = $coderow['code'];
