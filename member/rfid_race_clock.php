...
<?php 
    ...
    $trquery = "select * from race where cid = '$cid' and id = '$tid'";
...
<p><strong>Category:</strong> 
  <?php 
    $cat_query ="SELECT cat FROM race_category where id = '".$cat_id."' and cid = '".$cid."'";  
...
<div class="container-fluid">
  ...
  <tfoot>
    ...
    <?php                        
        ...
        $dsql = "SELECT *, (TIME_TO_SEC(time_arrived) - TIME_TO_SEC(time_release))/60 as minutes, distance/((TIME_TO_SEC(time_arrived) - TIME_TO_SEC(time_release))/60) as speed FROM race_result where rid = '".$_GET['rfid_id']."' and cid = '".$cid."' order by speed desc";
        $dresult = mysqli_query($db,$dsql);
         $sno = 1;

        while($dfetch = mysqli_fetch_array($dresult)){
        $dring_nr = strtoupper(base64_decode($dfetch['ring_nr']));
        $dmember_id = strtoupper(base64_decode($dfetch['my_club_id']));
...
$sql="select * from race_entries where code = '".$rfid."' and rid = '".$rrid."' and cid = '".$rcid."'";
$res=$db->query($sql);
$row=$res->fetch_assoc();
$entry_id = $row["id"];
$entry_ring_nr = $row["ring_nr"];
$entry_mem_id = $row["member_id"];
$entry_cid = $row['cid'];

$sql2="select * from club_members where cid = '".$rcid."' and id = '".$entry_mem_id."'";
$res2=$db->query($sql2);
$row2=$res2->fetch_assoc();
$mem_id = $row2["id"];
$mem_club_id = $row2["member_club_id"];
$mem_name = $row2["name"];
$mem_loft = $row2["loft_name"];
$mem_lat = $row2["coord_lat"];
$mem_lon = $row2["coord_long"];
$mem_cid = $row2['cid'];



$sql3="select * from race where cid = '".$rcid."' and id = '".$rrid."'";
$res3=$db->query($sql3);
...
$sql5="select id from race_entries where rid = '".$rrid."' and clock = 0 and cid = '".$rcid."'";
$res5=$db->query($sql5);
$row5=$res5->fetch_assoc();
$hidid = $row5["id"];

$check2=mysqli_query($db,"select * from race_entries where rid='".$rrid."' and cid = '".$rcid."' and code = '".$rfid."'");
...
die("Connection failed: " . mysqli_connect_error());
   ...
   $check=mysqli_query($db,"select rid, cid from race_result where rid='".$rrid."' and cid='".$rcid."' and code = '".$rfid."'");
  ...
  $mem_distance = calculateDistanceBetweenTwoPoints(base64_decode($r_lat), base64_decode($r_lon), base64_decode($mem_lat), base64_decode($mem_lon),'MT',true,5);
    //insert race result
    $sql6 = 'INSERT INTO race_result (uid, member_id, cid, rid, ring_nr, club, type, race_point, pt_coord_lat, pt_coord_long, my_club_id, name, loft_name, my_coord_lat, my_coord_long, time_release, time_arrived, distance, code) VALUES ("'.$ruid.'","'.$entry_mem_id.'","'.$rcid.'","'.$rrid.'","'.$entry_ring_nr.'","'.$c_club.'","'.$r_type.'","'.$r_point.'", "'.$r_lat.'", "'.$r_lon.'", "'.$mem_club_id.'", "'.$mem_name.'", "'.$mem_loft.'", "'.$mem_lat.'", "'.$mem_lon.'", "'.$r_time_release.'", SYSDATE(), "'.$mem_distance.'", "'.$rfid.'")';
     
    $result6 = mysqli_query($db, $sql6) or die('Error querying database.');

    //insert training
    $sql7 = "UPDATE race_entries SET clock = 1 where id = '".$hidid."'";     
