...
header("refresh:0; url=admin");
 ...
 $club_id = mysqli_real_escape_string($db, $_POST["club"]);
 
 $typeid = mysqli_real_escape_string($db, 2);
...
$check=mysqli_query($db,"select * from user where username ='$encrypt_unames'");
   ...
   if($checkrows>0) {
    echo "<script type= 'text/javascript'>alert('Username already exists in your records!');</script>";
    header("refresh:0; url=add-brgy-user");
    } else {
        if ($pass2<>$pass) {
    ...
    $stmt = $db->prepare("INSERT INTO user (type_id, username, password, fullname, un, club_id,date_subscribe, date_expired) VALUES (?,?,?,?,?,?,sysdate(),?)");
    $stmt->bind_param("issssis", $typeid, $encrypt_unames, $encrypt_pass, $encrypt_name, $encrypt_un, $club_id, $date_time_expire);   
    $stmt->execute();  
    $stmt->close();
