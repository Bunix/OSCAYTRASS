...
$sql_query3="DELETE FROM training_result WHERE uid='".$delete_id."'";
 ...
 header("Refresh: 0; url=active-subscribers");
}



  $query ="SELECT *, DATEDIFF(date_expired,sysdate()) as days FROM user where type_id = '$member_subscriber' and date_expired > sysdate() order by days";  
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        ...
        <li><a href="active-subscribers">Refresh</a></li>
    </div>
  </div>
</nav>
...
<!-- Begin Page Content -->
        <div class="container-fluid">
          ...
          <h1 class="h3 mb-2 text-gray-800">Active Subscribers</h1>
          <!-- DataTales -->
              ...
              <div class="table-responsive">
                <table class="table table-striped table-bordered" id="dataTable" width="100%" cellspacing="0"s>
                  ...
                  <form>
                          ...
                          {  
                                    $uid = $row['id']; 
                                    $encrypt_uid = urlencode(base64_encode($row['id']));
                                    $qry2 = "SELECT count(*) as cuid from p_details where uid = $uid";
                               ...
                               <tr>  
                                    ...
                                    <td style="text-align: center;"><a href="member-pigeons?id='.$encrypt_uid.$row['un'].'" class="trigger-btn" >'.$row2["cuid"].'</a></td>
                                    <td>'.number_format($row["no_records"]).'</td>
                                    <td>
            <a href="edit-member-subscription?id='.$encrypt_uid.$row['un'].'" class="trigger-btn" >Renew</a><br><br>
            <a href="change-member-password?id='.$encrypt_uid.$row['un'].'" class="trigger-btn" >Change Password</a><br><br>
...
function delete_id(id)
{
     ...
     {
        window.location.href='active-subscribers?delete_id='+id;
     }
}
</script>